export async function appendExpensesOrIncome(
  values: string[][],
  sheetId: number,
  sheetName: string,
): Promise<any> {
  console.log("appendExpensesOrIncome", {sheetId, values});
  console.time("appendExpensesOrIncome");

  const log = (text: string) => {
    console.time("Done: " + text);
    console.log("appendExpensesOrIncome", text);
  };
  ...
  try {
    ...
    const valuesUpdateBody: gapi.client.sheets.BatchUpdateValuesRequest = {
      ...
      data: [
        {
          range: `${sheetName}!C2:H${values.length + 1}`,
          values,
        },
      ],
    };
    ...
    console.timeEnd("appendExpensesOrIncome");
  }
}
